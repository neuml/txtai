...
class Expression:
    ...
    def readtoken(self, iterator):
        """
        Reads the next token from the token iterator. Raises a SQLError if the iterator has no more tokens.

        Args:
            iterator: token iterator

        Returns:
            (x, token) for the next token
        """
